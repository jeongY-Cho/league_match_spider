...
beforeAll(() => {
  ...
  fetchAccountInfo(testCases[0][0], testCases[0][1] as ValueOfRegions);
  process.env.RIOT_API_KEY = testCases[1][2];
  fetchAccountInfo(testCases[1][0], testCases[1][1] as ValueOfRegions);
  process.env.RIOT_API_KEY = testCases[2][2];
  fetchAccountInfo(testCases[2][0], testCases[2][1] as ValueOfRegions);
});
...
test("test proper endpoint call", () => {
  ...
  expect(calls[1][0]).toBe(
    url.resolve(region1,endpoints.ACCOUNT_INFO) + id2
  );
  expect(calls[2][0]).toBe(
    url.resolve(region2,endpoints.ACCOUNT_INFO) + id2
  );
});
test("test proper id append", () => {
  ...
  expect(calls[0][0]).toBe(
    url.resolve(region1,endpoints.ACCOUNT_INFO) + id1
  );
  expect(calls[1][0]).toBe(
    url.resolve(region1,endpoints.ACCOUNT_INFO) + id2
  );
