...
import url from "url";
import { fetchTimeline } from "../../fetchers/fetchTimeline";
...
beforeAll(() => {
  ...
  fetchTimeline(testCases[0][0] as number, testCases[0][1] as ValueOfRegions);
  process.env.RIOT_API_KEY = testCases[1][2] as string;
  fetchTimeline(testCases[1][0] as number, testCases[1][1] as ValueOfRegions);
  process.env.RIOT_API_KEY = testCases[2][2] as string;
  fetchTimeline(testCases[2][0] as number, testCases[2][1] as ValueOfRegions);
});
...
test("test proper endpoint call", () => {
  ...
  expect(calls[1][0]).toBe(url.resolve(region1, endpoints.TIMELINE) + match2);
  expect(calls[2][0]).toBe(url.resolve(region2, endpoints.TIMELINE) + match2);
});
test("test proper id append", () => {
  let calls = mocked(axios.get).mock.calls;

  expect(calls[0][0]).toBe(url.resolve(region1, endpoints.TIMELINE) + match1);
  expect(calls[1][0]).toBe(url.resolve(region1, endpoints.TIMELINE) + match2);
});
